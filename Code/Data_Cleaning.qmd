...
I used this qmd to work through fixing the rows that had issues in the duration column, as well as other smaller typos that came up. I referenced the data I had from Jackie's project, the raw online data provided by Sue Willson, and the original paper copies of that data provided by Sue Willson. 

By the end of this qmd, all issues that I noticed are fixed and I wrote a new csv file title ForgBout_final.csv to be used for future work. 
...
# 248 rows from 2001 - 250 once fix the last two rows
...
goodbird |>
  slice(c(308, 120))
# fixed them in the FixedDuration files, now need to run the code again for converting to seconds/minutes
```

### Read in the updated data to make final changes and save
```{r}
finalbird <- read.csv(here("Data/ForgBout_FixedDuration.csv"))
```
...
hms_duration_period <- hms(finalbird$duration)

finalbird$duration_seconds <- period_to_seconds(hms_duration_period)

## now convert to right units by dividing by 60 (above is still in milliseconds) ##

finalbird$duration_seconds <- finalbird$duration_seconds/60


## also want duration in minutes as a decimal because makes more sense for interpreting and can use as offset ##

finalbird$duration_minutes <- finalbird$duration_seconds/60

finalbird <- finalbird |> relocate(duration_minutes, .after = duration)
...
# it says there are no rows that end with something other than 00 - all set!
```
...
write.csv(finalbird, here("Data", "ForgBout_final.csv"))
```
