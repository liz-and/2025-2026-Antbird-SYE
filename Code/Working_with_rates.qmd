...
bird <- read.csv(here("Data/ForgBout_almost.csv"))
```
...
_New questions_
- Should i use the count variables or the T/F variables for models?
- 

### Thinking through approach to models
- Could do just one species at a time - might make the most sense because they're all different in dominance
...
```{r}
Phleg_mod_offset <- glm.nb(Successes ~ Rintra + Rinter + Iintra + Iinter + offset(log(duration_seconds)), data = Phleg_bird) 

summary(Phleg_mod_offset)
```
...
### code
```{r}
ZIPmod_Phleg <- zeroinfl(Successes ~ attempts + duration_seconds|Rintra + duration_seconds, data = Phleg_bird, dist = "poisson")

# now thinking that attempts and successes is way too correlated to use that as a predictor - do something else?

# worked
summary(ZIPmod_Phleg)
